...
def cmd_vel(robot: Robot, linear_vel: float, angular_vel: float) -> None:
    r_omega: float = min((linear_vel + angular_vel * AXLE_LENGTH / 2) / WHEEL_RADIUS,6.28)
    l_omega: float = min((linear_vel - angular_vel * AXLE_LENGTH / 2) / WHEEL_RADIUS,6.28)
...
def full_warp_robot_ai(supervisor: Supervisor, robot_def_name: str, new_position, new_orientation) -> None:
    robot_node = supervisor.getFromDef(robot_def_name)

    trans_field: Field = robot_node.getField("translation")
    translation: List[float] = [new_position[0], new_position[1], new_position[2]]
    trans_field.setSFVec3f(translation)
